...
def UserOptimized():
    ...
    cursor.execute("DROP INDEX IF EXISTS index_sel")
    cursor.execute("DROP INDEX IF EXISTS customer_id_index_ord_items")

    cursor.execute("CREATE INDEX index_sel ON Sellers( seller_id, seller_postal_code)")
    cursor.execute("CREATE INDEX customer_id_index_ord_items ON Order_items( order_id, seller_id)")
    ...
    for i in range(50):
        cursor.execute(''' SELECT COUNT( DISTINCT s.seller_postal_code)
                           ...
                           WHERE s.seller_id = oi.seller_id
                            AND order_id = ( 
                                   SELECT oi.order_id
                                   FROM Order_items oi ORDER BY random() LIMIT 1);''' )
